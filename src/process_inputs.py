...
# Building the index of the df assuming column G is empty (on all rows above data) and column G is the first time series column
...
def spreadsheet_to_df(file_name):
    ...
    # Build the index of the df assuming column G is empty and column G is the first time series column
    ...
    for cellObj in cell_names[(col - 1) :]:
        try:
