...
from pages.utils.process_inputs import stringify,show_df
...
def main():
    ...
    #base year
    print(st.session_state.df[st.session_state.y_sel][base_year_start:base_year_end+1])
    ...
    st.header("Base year data:")
    st.dataframe(st.session_state.df[st.session_state.y_sel][base_year_start:base_year_end+1])
    st.header("Growth rates:")
    #growth rate of GDP
    elast_df = st.session_state.g_df[st.session_state.x_sel_g] ** edited_df[st.session_state.x_sel_g].iloc[0][
        st.session_state.x_sel_g]
    st.dataframe(elast_df)
    # st.write(edited_df[st.session_state.x_sel_g].iloc[0][st.session_state.x_sel_g])
    st.header("Backcast:")
    st.session_state.bc_df = pd.DataFrame(data = elast_df.shift(periods=-prd),index=st.session_state.df.index)
    ...
    st.session_state.bc_df.loc[st.session_state.bc_df.index[-prd:], 'Cumulative Growth'] = 1
    ...
    for col in st.session_state.x_sel_g:
        for i in range(len(st.session_state.bc_df) - 5, -1, -4):
            df_reset.loc[i, 'Cumulative Growth'] = df_reset.loc[i, col] * df_reset.loc[i+4, col]
            df_reset.loc[i-1, 'Cumulative Growth'] =  df_reset.loc[i-1, col] * df_reset.loc[i+3, col]
            df_reset.loc[i-2, 'Cumulative Growth'] = df_reset.loc[i-2, col] * df_reset.loc[i+2, col]
            df_reset.loc[i-3, 'Cumulative Growth'] = df_reset.loc[i-3, col] * df_reset.loc[i+1, col]
    ...
    for i in range(0,len(st.session_state.bc_df)):
        df_reset.loc[i, 'Predicted y'] = df_reset.loc[i, 'Cumulative Growth'] * st.session_state.df[st.session_state.y_sel][i]
    df_reset['Predicted y'] = df_reset['Predicted y'].astype(float)
    st.session_state.bc_df = st.session_state.bc_df.set_index('index')
    st.session_state.bc_df[st.session_state.y_sel] = st.session_state.df[st.session_state.y_sel]
    st.dataframe(st.session_state.bc_df)
    ...
    st.session_state.bc_plot_df = st.session_state.bc_df[['Predicted y',st.session_state.y_sel]]
    fig = px.line(st.session_state.bc_plot_df, x=st.session_state.bc_plot_df.index, y=st.session_state.bc_plot_df.columns,title=f"Backcasting {st.session_state.y_sel} over time")
    fig.update_layout(
        xaxis_title="Year",
