...
from pages.utils.process_inputs import stringify,stringify_g_df
...
def main():
    ...
    if st.session_state.g_df is None and prd is None:
        ...
        st.session_state.g_df,st.session_state.g_df_idx = growth_df(st.session_state.df)
    ...
    st.session_state.y_sel_g = st.selectbox('Choose the dependent (endogenous) variable:',options=y_cols)
    st.session_state.x_sel_g = st.multiselect('Choose independent (exogenous) variables:',options=x_cols) #,on_change=show_df,
    ...
    if st.button('Update dataframe'):
        show_dataframe(st.session_state.g_df, st.session_state.y_sel_g, st.session_state.x_sel_g, st.session_state.slider_value_start, st.session_state.slider_value_end)
    try:
        if st.session_state.y_sel_g != None and st.session_state.x_sel_g != None:
            if st.session_state.r_df is not None:
                y = st.session_state.r_df[st.session_state.y_sel_g][st.session_state.slider_value_start:st.session_state.slider_value_end]
                if constant_sel == 'Yes':
                    X = st.session_state.r_df[st.session_state.x_sel_g][st.session_state.slider_value_start:st.session_state.slider_value_end]
