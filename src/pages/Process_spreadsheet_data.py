...
def main():
    st.set_page_config(
        page_title="Process spreadsheet data"
    )
    # initialize_session_state()
    st.sidebar.success("In this page, the user uploads the data for review before any calculation is performed")
    st.header("Upload a filled version of the template spreadsheet:")
    filename = st.text_input("Enter the filename (with extension) saved in data/reg_input folder:")
    print(filename)
