...
def main():
    ...
    if st.session_state.model_regressions_df is not None:
        df = st.session_state.model_regressions_df.iloc[
        ...
        tvalues_df = st.session_state.regression_outputs[st.session_state.reg_sel][
            "t stats"
        ]
    ...
    if st.button("Export output spreadsheet with selected regression"):
        ...
        for test in tests:
            ...
            residuals_df["Actuals"] = (
                residuals_df["Residuals"] + residuals_df["Fitted values"]
            )
